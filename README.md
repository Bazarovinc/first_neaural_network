...
```
## И еще...
Нейронная сеть это лишь программная реализация математической модели, так что ее можно реализовать не только на Python'е, но и на любом другом языке программирования. Так я реализовал нейронную сеть в программе Matlab, которая лежит по этой [ccылке](https://github.com/Bazarovinc/first_neaural_network_matlab.git).
